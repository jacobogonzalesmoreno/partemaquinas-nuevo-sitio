...
export default function CargaMasivaPage() {
  ...
  const subirUnaImagen = async (file, indiceGlobal, intentos = 5) => {
    ...
    for (let intento = 1; intento <= intentos; intento++) {
      try {
        ...
        if (!res.ok) {
          ...
          throw new Error(data.error || `Error del servidor (${res.status}) al subir imagen`);
        }
        const data = await res.json();
        return data.url;
      } catch (err) {
        if (intento === intentos) {
          throw new Error(`Foto ${indiceGlobal + 1}: ${err.message || 'fallo de conexion'}`);
        }
  ...
  const subirEnLotes = async (archivos, concurrencia = 1) => {
    ...
    const trabajadores = Array.from({ length: Math.min(concurrencia, archivos.length) }, async () => {
      while (indice < archivos.length) {
        const i = indice++;
        urls[i] = await subirUnaImagen(archivos[i], i);
      }
    });
  ...
  const crearTodos = async () => {
    ...
    for (const lote of lotes) {
      try {
        ...
        if (!res.ok) {
          const data = await res.json().catch(() => ({}));
          throw new Error(data.detalle || data.error || 'No se pudo crear el producto.');
        }
