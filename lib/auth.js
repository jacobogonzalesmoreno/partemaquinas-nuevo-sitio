...
function resolveRoleFromToken(token) {
  ...
  if (ADMIN_API_KEY && timingSafeEqual(token, ADMIN_API_KEY)) return 'admin';
  if (EDITOR_API_KEY && timingSafeEqual(token, EDITOR_API_KEY)) return 'editor';
  if (READER_API_KEY && timingSafeEqual(token, READER_API_KEY)) return 'reader';
  return null;
}
...
export function requireRole(request, minRole) {
  ...
  if (session && !['GET', 'HEAD', 'OPTIONS'].includes(method)) {
    ...
    if (!origin || origin !== new URL(request.url).origin) {
      return { ok: false, response: NextResponse.json({ error: 'Origen no permitido.' }, { status: 403 }) };
    }
  }
