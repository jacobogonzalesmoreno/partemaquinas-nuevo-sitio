  ...
  if (!cachedSecurityEnv) {
    ...
    if (!parsed.success) {
      const details = parsed.error.issues
        .map(issue => `${issue.path.join('.') || 'Configuracion'}: ${issue.message}`)
        .join(', ');
      throw new Error(`Configuracion de seguridad invalida: ${details}`);
    }
    const isProd = process.env.NODE_ENV === 'production';
    if (isProd && (
      !parsed.data.ADMIN_USER ||
      !parsed.data.ADMIN_PASSWORD ||
      parsed.data.ADMIN_PASSWORD.length < 16 ||
      ['adminparte', 'admin', 'admin123', 'password', 'changeme'].includes(parsed.data.ADMIN_PASSWORD.toLowerCase()) ||
      !parsed.data.SESSION_SECRET ||
      parsed.data.SESSION_SECRET.length < 32
    )) {
      throw new Error('Configura ADMIN_USER, una contraseña administrativa única de al menos 16 caracteres y SESSION_SECRET aleatorio de al menos 32 caracteres.');
    }
    cachedSecurityEnv = {
      ...parsed.data,
