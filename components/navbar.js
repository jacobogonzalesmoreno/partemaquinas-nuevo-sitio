...
export default function Navbar() {
  ...
  const onFocusBuscar = () => {
    if (buscarNav.trim().length >= 2 && (sugerenciasCats.length > 0 || sugerenciasProds.length > 0)) setMostrarSugerencias(true);
  };
  ...
  const onKeyDownBuscar = event => {
    ...
    switch (event.key) {
      ...
      case 'Enter': {
        ...
        if (idx >= 0 && idx < sugerenciasCats.length) { router.push(sugerenciasCats[idx].href); setBuscarNav(''); setMostrarSugerencias(false); return; }
        const prodIdx = idx - sugerenciasCats.length;
        if (prodIdx >= 0 && prodIdx < sugerenciasProds.length) { router.push(`/productos/${sugerenciasProds[prodIdx].id}`); setBuscarNav(''); setMostrarSugerencias(false); return; }
        if (idx === sugerenciasCats.length + sugerenciasProds.length) { onSubmitBuscar(event); return; }
    ...
    return (
      <div className="absolute left-0 top-full z-[100] mt-1 w-80 sm:w-96 rounded-xl border border-slate-200 bg-white shadow-xl overflow-hidden">
        <ul className="py-1 max-h-[70vh] overflow-y-auto">
          {tieneCats && (<li><p className="px-3.5 py-1.5 text-[10px] font-semibold uppercase tracking-[0.2em] text-slate-400 bg-slate-50/80">Categorias</p></li>)}
          {sugerenciasCats.map((sug) => {
            ...
            return (
              <li key={`cat-${sug.href}`}>
                <button type="button" className={`w-full text-left px-3.5 py-2.5 flex items-center gap-3 transition-colors ${isActive ? 'bg-amber-50 text-slate-900' : 'text-slate-700 hover:bg-slate-50'}`}
                  onMouseDown={event => { event.preventDefault(); router.push(sug.href); setBuscarNav(''); setMostrarSugerencias(false); }}
                  ...
                  <span className={`text-[9px] font-semibold uppercase tracking-wider px-1.5 py-0.5 rounded-full flex-shrink-0 ${sug.nivel === 0 ? 'bg-amber-100 text-amber-700' : 'bg-orange-100 text-orange-600'}`}>{sug.tipo}</span>
                </button>
              </li>
            );
          })}
          {tieneCats && tieneProds && <li className="border-t border-slate-100" />}
          {tieneProds && (<li><p className="px-3.5 py-1.5 text-[10px] font-semibold uppercase tracking-[0.2em] text-slate-400 bg-slate-50/80">Productos</p></li>)}
            ...
            return (
              <li key={`prod-${prod.id}`}>
                <button type="button" className={`w-full text-left px-3.5 py-2.5 flex items-center gap-3 transition-colors ${isActive ? 'bg-amber-50 text-slate-900' : 'text-slate-700 hover:bg-slate-50'}`}
                  ...
                  onMouseEnter={() => setSugerenciaActiva(thisIndex)}>
                  {prod.imagen ? <img src={prod.imagen} alt="" className="h-10 w-10 rounded-lg object-cover bg-slate-100 border border-slate-200 flex-shrink-0" /> : <span className="flex items-center justify-center h-10 w-10 rounded-lg bg-slate-100 border border-slate-200 text-slate-400 text-lg flex-shrink-0">⚙️</span>}
                  <div className="flex-1 min-w-0">
                  ...
                  <ul className="space-y-0.5">
                    {categoria.hijos.map(hijo => (
                      <li key={hijo.nombre} className="relative group/child z-10 hover:z-30"
                        onMouseEnter={event => { if (hideSubcategoriaTimer.current) { clearTimeout(hideSubcategoriaTimer.current); hideSubcategoriaTimer.current = null; } setSubcategoriaActiva(hijo.nombre); }}
                        onMouseLeave={() => { if (hideSubcategoriaTimer.current) clearTimeout(hideSubcategoriaTimer.current); hideSubcategoriaTimer.current = setTimeout(() => setSubcategoriaActiva(null), 150); }}>
                        ...
                        {hijo.hijos && (
                          <div className={`absolute left-[calc(100%+14px)] md:left-[calc(100%+18px)] top-0 z-[70] w-48 transition-all duration-200 ease-out ${subcategoriaActiva === hijo.nombre ? 'opacity-100 translate-x-0 scale-100 pointer-events-auto' : 'opacity-0 translate-x-2 scale-95 pointer-events-none'}`}>
      ...
      <div className="relative flex items-center justify-between">
        ...
        <div className="hidden md:flex gap-6 text-sm font-semibold absolute left-1/2 -translate-x-1/2">
          ...
          <div className="relative group">
            <Link href="/productos" className={`${navLinkClass} gap-1`}>Productos<span className="text-orange-400">▾</span></Link>
        ...
        <div ref={searchRef} className="hidden lg:block relative">
          <form onSubmit={event => onSubmitBuscar(event)} className="flex items-center gap-2">
            <div className="relative">
              ...
              <input type="text" value={buscarNav} onChange={onChangeBuscar} onFocus={onFocusBuscar} onBlur={onBlurBuscar} onKeyDown={onKeyDownBuscar} placeholder="Buscar producto o categoría..." autoComplete="off" role="combobox" aria-expanded={mostrarSugerencias} aria-autocomplete="list"
            ...
            <button type="submit" className="btn-anim inline-flex items-center justify-center rounded-xl bg-slate-900 text-white px-3 py-2 text-sm shadow-sm ring-1 ring-amber-200 transition-all hover:border-amber-300 hover:bg-slate-800 hover:shadow-md hover:ring-2 hover:ring-amber-300" aria-label="Buscar">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
            </button>
          </form>
        </div>
